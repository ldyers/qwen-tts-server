...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id, abogus_file=None):
        """
        直播间弹幕抓取对象
        ...
        # 读取用户登录 Cookie（用于让服务器下推礼物流等完整消息）
        # 优先环境变量 DOUYIN_COOKIE，其次文件 /app/data/cookie.txt
        self.user_cookie = ""
        import os as _os
        env_cookie = _os.environ.get('DOUYIN_COOKIE', '').strip()
        if env_cookie:
            self.user_cookie = env_cookie
        else:
            for _p in ('/app/data/cookie.txt', 'data/cookie.txt', 'cookie.txt'):
                try:
                    if _os.path.exists(_p):
                        with open(_p, 'r', encoding='utf-8') as _f:
                            self.user_cookie = _f.read().strip()
                        if self.user_cookie:
                            print(f"【√】已加载登录 Cookie（来自 {_p}，长度 {len(self.user_cookie)}）")
                            break
                except Exception as _e:
                    print(f"【X】读取 Cookie 文件 {_p} 失败: {_e}")
    ...
    def get_room_status(self):
        ...
        if data:
            ...
            # 保存直播间信息（主播昵称、房间标题）供 Web 后台显示
            room = data.get('data')
            title = ""
            if isinstance(room, list) and room:
                title = room[0].get('title', "") or ""
    ...
    def _connectWebSocket(self):
        ...
        wss = ("wss://webcast100-ws-web-lq.douyin.com/webcast/im/push/v2/?app_name=douyin_web"
               ...
               f"&cursor=d-1_u-1_fh-0_t-{int(time.time()*1000)}_r-1"
        ...
        # 拼接 Cookie：优先使用用户登录 Cookie（含 sessionid 才能收到礼物流），否则退回匿名 ttwid
        if self.user_cookie:
            cookie_str = self.user_cookie
            if 'ttwid=' not in cookie_str and self.ttwid:
                cookie_str = f"ttwid={self.ttwid}; " + cookie_str
        else:
