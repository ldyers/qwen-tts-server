...
class Settings(BaseSettings):
    ...
    # VPS 模式下，推理委托给远程 GPU Worker
    remote_mode: bool = Field(
        default=False,
        description="Enable remote worker mode (VPS -> GPU Worker over WireGuard)"
    )
    worker_url: str = Field(
        default="http://10.100.0.2:8001",
        description="GPU Worker URL (Windows machine on WireGuard)"
    )
    worker_api_key: str = Field(
        default="",
        description="Shared API key for Worker authentication"
    )
    worker_timeout: float = Field(
        default=120.0,
        description="HTTP timeout for Worker requests (seconds)"
    )
