...
**弹幕播报优化（基于 311 次 TTS 日志数据分析）**

- **纯表情弹幕过滤**：`[捂脸][赞]` 等无文字内容不合成（节省 16.5%）
- **乱码/生僻字过滤**：中文占比 <30% 的弹幕不合成
- **重复弹幕去重**：10 秒内相同内容只合成一次
- **重复字弹幕截断**：连续重复 3 次以上的字符截断为 2 个（`嘎嘎嘎嘎` → `嘎嘎`）
- **TTS 结果缓存**：5 分钟内相同文本复用音频，最多缓存 50 条
- **状态面板增强**：缓存命中数、节省时间、过滤详情分类显示
- **Windows 桌面快捷方式**：双击「释放显存」停止 Worker 释放 GPU 显存
